...
public class UpdateUserEmail extends AppCompatActivity {
        ...
        docRef.get().addOnCompleteListener(new OnCompleteListener<DocumentSnapshot>() {
            ...
            public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                ...
                if (task.isSuccessful())
                {
                    ...
                    if (doc.exists())
                    {
                        Log.d("Display email", "DocumentSnapshot data: " + doc.getData());
                        etUpdateEmail.setText(doc.getData().get("eMail").toString());

                    }
                    else {
                        Toast.makeText(getApplicationContext(), "Failed to get your email", Toast.LENGTH_SHORT).show();
                    }
                }
                else
                {
    ...
    private void updateUserEmail() {
        ...
        if(!etUpdateEmail.getText().toString().isEmpty()){
            DocumentReference nameRef = db.collection("users").document(FirebaseAuth.getInstance().getCurrentUser().getUid());

            nameRef
                    .update("eMail", etUpdateEmail.getText().toString())
                    .addOnSuccessListener(new OnSuccessListener<Void>() {
                        @Override
                        public void onSuccess(Void aVoid) {
                            Log.d("EditEmail", "DocumentSnapshot successfully updated!");
                    ...
                    .addOnFailureListener(new OnFailureListener() {
                        ...
                        public void onFailure(@NonNull Exception e) {
                            Toast.makeText(UpdateUserEmail.this, "Failed to update.", Toast.LENGTH_SHORT).show();
                        }
                    });
        }
        else {
            Toast.makeText(UpdateUserEmail.this, "This field cannot be blank.", Toast.LENGTH_SHORT).show();
        }
